...
from app.results.presentation import CHANNEL_LABELS, CHANNEL_ORDER
from app.results.scoring import build_compilation_rows


def build_participant_placeholders(participant, edition_id):
    """Retourne {nom de balise: valeur} pour ce participant."""
    ...
    if row:
        for channel in CHANNEL_ORDER:
            ...
            values[f"Nb tests {label}"] = channel_data["nb_test"]
            values[f"Note {label}"] = channel_data["note_20"] if channel_data["note_20"] is not None else "—"
